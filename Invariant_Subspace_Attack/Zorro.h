#ifndef  ZORRO_H
#define ZORRO_H

#include <iostream>
#include <string.h>		// For memcpy()

#include "Cipher.h"

// We're going to implement the working of Zorro as a C++ class



class Zorro : public Cipher
{
	public:
		void init() override;		// Initialization for running Zorro
		void round_constant(void* data, int i) override;	// Adds i^th round constant to data
		void round_function(void* data) override;		//Performs Zorro round function

		void print_state(void* data) override;

		void encrypt(uint8_t plaintext[4][4], uint8_t ciphertext[4][4], uint8_t key[4][4], int rounds);

	private:
		// A Zorro cipher encryption consists of 6 steps
		void zorro_step(uint8_t state[4][4], uint8_t key[4][4], int round);

		// Each Zorro step consists of 4 rounds
		void zorro_round(uint8_t state[4][4], int round);

		// A Zorro round is made up of 4 transformations
		void SB(uint8_t state[4][4]);
		void AC(uint8_t state[4][4], int round);
		void SR(uint8_t state[4][4]);
		void MC(uint8_t state[4][4]);

		// ...and an Key adding step:
		void AK(uint8_t state[4][4], uint8_t key[4][4]);
		
		// Some Matrices that are used in Zorro transformations
		uint8_t zorro_sbox[256] = {
		 };

		uint8_t GF_mul2[256] = {
		
		};

		uint8_t GF_mul3[256] = {
		
		};

};

#endif // !ZORRO_H
